...
function renderCompare(){
  ...
  if(typeof applyFilter==='function') applyFilter();   // refresh the "N of N renders" count
}
...
function applyFilter(){{
  ...
  if(cEl){{
    // dropped renders live in their own container, outside cardsAll - count them too, or the
    // standalone viewer reads "0 of 0 renders" while a render you just dropped is on screen
    var nDrop=(typeof dropped!=='undefined' && dropped) ? dropped.length : 0;
    cEl.textContent='';
    var b=document.createElement('b'); b.textContent=shown+nDrop;
    cEl.appendChild(b); cEl.appendChild(document.createTextNode(' of '+(cardsAll.length+nDrop)+' renders'));
  }}
